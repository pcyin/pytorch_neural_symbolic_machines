...
def normalize(x):
    # text is encoded by default as unicode in Python 3
...
def main():
    ...
    for filename in os.listdir(args.tagged_dataset_path):
        ...
        print('Reading dataset from {}'.format(filename), file=sys.stderr)
    ...
    print('Read {} examples'.format(len(target_values_map)), file=sys.stderr)

    print('Reading predictions from {}'.format(args.prediction_path), file=sys.stderr)
    ...
    with open(args.prediction_path, 'r', 'utf8') as fin:
        for line in fin:
            ...
            if ex_id not in target_values_map:
                ...
                if correct:
                    num_correct += 1
    print('Examples: {}'.format(num_examples), file=sys.stderr)
    print('Correct: {}'.format(num_correct), file=sys.stderr)
